...
export function Sidebar() {
  ...
  return (
    <aside
      ...
      className={`border-r dark:border-gray-800 h-screen flex flex-col transition-all duration-300 ${open ? "w-64" : "w-20"}`}
    >
      {/* HEADER */}
      <div
        className={`flex items-center justify-between p-4 border-b dark:border-gray-800 ${open ? "" : "justify-center"}`}
      >
        {open && (
        ...
        <nav className="flex-1 space-y-2 p-4">
          {menuItems.map((item) => {
            ...
            return (
              ...
              >
                <Icon size={20} style={{ color: isActive ? "white" : "var(--foreground)" }} />
                <span className="text-sm font-medium">{item.label}</span>
                {isActive && <span className="absolute right-3 w-2 h-2 bg-white rounded-full" />}
              </Link>
            );
          })}
        </nav>
      ...
      {/* FOOTER */}
      {open && (
        <div className="mt-6 text-xs p-4" style={{ color: "var(--muted)" }}>
          v0.1.0
        </div>
      )}
    </aside>
